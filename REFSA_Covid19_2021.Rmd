...
date_from <- ymd("2021-08-15")
date_to <- ymd("2022-02-09")
...
sc_asean_avg <- school_closure %>% filter(region == "ASEAN") %>% summarise(country = "ASEAN", iso = NA, region = "ASEAN", across(pandemic_total:partial_sep21_oct21, mean))

# Calculate average closure durations for the world
sc_world_avg <- school_closure %>% summarise(country = "World", iso = NA, region = "World", across(pandemic_total:partial_sep21_oct21, mean))
...
# Plot
sc_p <- ggplot(data = sc %>% filter(country %in% c("Malaysia", "ASEAN", "World") & type %in% c("full", "partial") & period == "pandemic"), aes(x = factor(country, levels=c("Malaysia", "ASEAN", "World"), ordered = TRUE))) +
