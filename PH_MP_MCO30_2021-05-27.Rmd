...
daily_cases_my <- daily_cases_my %>% mutate(mco = case_when(
  (date >= ymd("2020-03-03") & date < ymd("2020-05-18")) ~ "MCO1", 
  (date >= ymd("2021-05-11") & date < ymd("2021-06-30")) ~ "MCO3")) 
...
daily_cases_my_plot <- ggplot(data = filter(daily_cases_my, mco != "NA"), aes(x = date)) +
  ...
  geom_vline(xintercept = ymd(lockdowns %>% filter(lockdown == "MCO-3") %>% pull(end)), color = "#999999") +
...
covid_mco_tests_m <- covid_mco_tests_m %>% mutate(mco = case_when(
  (record.date >= ymd("2020-03-03") & record.date < ymd("2020-05-18")) ~ "MCO1", 
  (record.date >= ymd("2021-05-11") & record.date < ymd("2021-06-30")) ~ "MCO3")) 

# Plot daily tests (per thousand)
covid_mco_dailytests_plot <- ggplot(data = filter(covid_mco_tests_m, mco != "NA"), aes(x = record.date)) + 
  geom_vline(xintercept = ymd(lockdowns %>% filter(lockdown == "MCO-1") %>% pull(start))) +
  geom_vline(xintercept = ymd(lockdowns %>% filter(lockdown == "MCO-1") %>% pull(end))) +
  geom_vline(xintercept = ymd(lockdowns %>% filter(lockdown == "MCO-3") %>% pull(start))) +
  geom_vline(xintercept = ymd(lockdowns %>% filter(lockdown == "MCO-3") %>% pull(end))) +
  geom_bar(aes(y = daily.tests.per.thousand), stat = "identity") + 
  scale_x_date(name = "Date", date_breaks = "7 days", date_labels = "%d-%b-%y") +
  scale_y_continuous(name = "Count") + 
...
covid_mco_tpr_plot + facet_wrap(~ mco, scales = "free_x")
...
mobi_plot <- ggplot() +
  ...
  geom_vline(xintercept = ymd(lockdowns %>% filter(lockdown == "MCO-3") %>% pull(start)), color = "#999999") +
  geom_vline(xintercept = ymd(lockdowns %>% filter(lockdown == "MCO-3") %>% pull(end)), color = "#999999") +
  geom_area(data = filter(mobi_mco, mco != "NA" & category %in% c("retail.rec", "grocery.pharmacy", "transit", "workplaces")), aes(x = date, y = change.from.baseline.smooth, color = category, fill = category), alpha = 0.33) +
...
#  geom_rect(data = filter(lockdowns, lockdown == "MCO-1"), inherit.aes = FALSE, aes(xmin = start, xmax = end, ymin = -Inf, ymax = +Inf), fill= 'red', alpha = 0.1) +
